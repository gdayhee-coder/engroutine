...
const C = {
  ...
  sage:"#9DB088", sageDark:"#7E9469", sageDeep:"#5E7049", sageLight:"#B9CBA4",
  gold:"#D9C286", coral:"#C99F86", blue:"#9DB8C4",
...
const Logo = ({ size = 40, radius }) => (
  <div style={{ width:size, height:size, borderRadius:radius ?? size*0.28, background:"#7E9469", display:"flex", alignItems:"center", justifyContent:"center", boxShadow:"0 2px 6px rgba(94,112,73,0.25)", flexShrink:0 }}>
    <svg width={size*0.62} height={size*0.62} viewBox="0 0 48 48">
      <path d="M24 15C20.5 12.8 14.5 12.8 11.8 13.5C11 13.7 10.3 14.4 10.3 15.3V29c0 1 .9 1.7 1.9 1.4C14.8 29.7 20.5 29.7 24 32V15Z" fill="#fff"/>
      <path d="M24 15C27.5 12.8 33.5 12.8 36.2 13.5C37 13.7 37.7 14.4 37.7 15.3V29c0 1-.9 1.7-1.9 1.4C33.2 29.7 27.5 29.7 24 32V15Z" fill="#EDF2E4"/>
      <path d="M32 8a10 10 0 019 6" stroke="#D9C286" strokeWidth="2.6" strokeLinecap="round" fill="none"/>
      <path d="M37.5 10.5l3.5 3.5-4.5 1" stroke="#D9C286" strokeWidth="2.6" strokeLinecap="round" strokeLinejoin="round" fill="none"/>
    </svg>
  </div>
);
...
const LINK_BOOKTUBE = "https://booktube.vercel.app";
...
export default function App() {
    ...
    return (
      <div style={{ fontFamily:FF, background:C.bg, minHeight:"100vh", display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:20 }}>
        ...
        <div style={{ width:"100%", maxWidth:320, background:C.paper, borderRadius:16, padding:20, border:`1.5px solid ${C.border}`, boxShadow:"0 4px 16px rgba(94,112,73,0.08)" }}>
          ...
          {!loginErr && <div style={{ fontSize:11, color:C.textMuted, marginTop:12, textAlign:"center", lineHeight:1.6 }}>선생님께 받은 아이디와 비밀번호로<br/>로그인하세요 🌿</div>}
        </div>
      </div>
    );
  }
  ...
  const Header = () => (
    <div style={{ background:`linear-gradient(160deg, #C7D8B4, #A9BE92)`, padding:"16px 20px" }}>
      <div style={{ maxWidth:720, margin:"0 auto", display:"flex", alignItems:"center", gap:11 }}>
        <Logo size={40} radius={11} />
        <div>
          <div style={{ fontSize:18, fontWeight:900, color:"#455239", letterSpacing:"-0.3px" }}>잉글루틴</div>
          <div style={{ fontSize:11, color:"rgba(69,82,57,0.7)" }}>👤 {user.name}님</div>
        </div>
        <button onClick={doLogout} style={{ marginLeft:"auto", padding:"7px 13px", background:"rgba(255,255,255,0.5)", color:"#455239", border:"none", borderRadius:20, fontSize:11.5, fontWeight:700, cursor:"pointer", fontFamily:FF }}>로그아웃</button>
      </div>
    </div>
  );
                ...
                <div style={{ display:"grid", gap:10 }}>
                  {books.map((b,i)=>(
                    <button key={i} onClick={()=>openBook(b)}
                      style={{ display:"flex", alignItems:"center", gap:12, background:C.paper, border:`1.5px solid ${C.border}`, borderLeft:`6px solid ${[C.sage,C.gold,C.coral,C.blue,C.sageLight][i%5]}`, borderRadius:12, padding:"14px 16px", cursor:"pointer", fontFamily:FF, textAlign:"left", boxShadow:"2px 3px 0 rgba(107,124,96,0.08)" }}>
            ...
            <div>
              ...
              {ytLoading ? (
                ...
                <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fill, minmax(220px, 1fr))", gap:14 }}>
                  {ytItems.map((v)=>(
                    <div key={v.id} onClick={()=>setPlaying(v)} style={{ background:C.paper, borderRadius:12, overflow:"hidden", border:`1px solid ${C.border}`, cursor:"pointer", boxShadow:"2px 3px 0 rgba(107,124,96,0.1)" }}>
  ...
  return (
    <div style={{ fontFamily:FF, background:C.bg, minHeight:"100vh" }}>
      ...
      <div style={{ maxWidth:720, margin:"0 auto", padding:"20px 20px 40px" }}>
        <div style={{ fontSize:15, fontWeight:900, color:C.text, marginBottom:3 }}>안녕하세요, {user.name}님 🌿</div>
        ...
        <div style={{ display:"grid", gap:12, marginBottom:18 }}>
          <a href={`${LINK_BOOKTUBE}?uid=${encodeURIComponent(user.id)}&upin=${encodeURIComponent(user.pin)}`} target="_blank" rel="noopener noreferrer"
            style={{ display:"flex", alignItems:"center", gap:14, background:C.paper, border:`1.5px solid ${C.border}`, borderLeft:`7px solid ${C.sage}`, borderRadius:14, padding:"18px 18px", textDecoration:"none", boxShadow:"2px 3px 0 rgba(107,124,96,0.1)" }}>
            {Ic.book(C.sage, 34)}
            <div style={{ flex:1 }}>
          ...
          <a href={LINK_AR} target="_blank" rel="noopener noreferrer"
            style={{ display:"flex", alignItems:"center", gap:14, background:C.paper, border:`1.5px solid ${C.border}`, borderLeft:`7px solid ${C.gold}`, borderRadius:14, padding:"18px 18px", textDecoration:"none", boxShadow:"2px 3px 0 rgba(107,124,96,0.1)" }}>
            {Ic.quiz(C.gold, 34)}
            <div style={{ flex:1 }}>
          ...
          <button onClick={()=>setView("books")}
            style={{ display:"flex", alignItems:"center", gap:14, background:C.paper, border:`1.5px solid ${C.border}`, borderLeft:`7px solid ${C.coral}`, borderRadius:14, padding:"18px 18px", cursor:"pointer", fontFamily:FF, textAlign:"left", boxShadow:"2px 3px 0 rgba(107,124,96,0.1)" }}>
            {Ic.study(C.coral, 34)}
            <div style={{ flex:1 }}>
        ...
        <button onClick={()=>setView("records")}
          style={{ width:"100%", background:C.paper, border:`1.5px solid ${C.border}`, borderRadius:14, padding:"15px 18px", cursor:"pointer", fontFamily:FF, textAlign:"left", boxShadow:"2px 3px 0 rgba(107,124,96,0.08)" }}>
