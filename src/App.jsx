...
export default function App() {
  ...
  // ===== 집중듣기 화면 (iframe) =====
  if (view === "listen") {
    const btUrl = `${LINK_BOOKTUBE}?uid=${encodeURIComponent(user.id)}&upin=${encodeURIComponent(user.pin)}`;
    return (
      <div style={{ fontFamily:FF, background:C.bg, minHeight:"100vh", display:"flex", flexDirection:"column" }}>
        <div style={{ background:`linear-gradient(160deg, #FFB199, #FF7A5C)`, padding:"10px 16px", display:"flex", alignItems:"center", gap:10 }}>
          ...
          <div style={{ fontSize:13.5, fontWeight:800, color:"#5A2E1F" }}>📖 집중듣기</div>
          <a href={btUrl} target="_blank" rel="noopener noreferrer"
      ...
      <div style={{ maxWidth:720, margin:"0 auto", padding:"20px 20px 40px" }}>
        ...
        <div style={{ display:"grid", gap:12, marginBottom:18 }}>
          <button onClick={()=>setView("listen")}
            style={{ display:"flex", alignItems:"center", gap:14, background:C.paper, border:`1.5px solid ${C.border}`, borderLeft:`7px solid ${C.sage}`, borderRadius:14, padding:"18px 18px", cursor:"pointer", fontFamily:FF, textAlign:"left", width:"100%", boxShadow:"2px 3px 0 rgba(255,122,92,0.12)" }}>
            {Ic.book(C.sage, 34)}
            <div style={{ flex:1 }}>
